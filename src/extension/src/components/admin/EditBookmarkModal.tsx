...
import { type PublicBookmark, type SharePool, publicBookmarkApi } from '../../api/publicBookmarks';
...
export const EditBookmarkModal: React.FC<EditBookmarkModalProps> = ({ isOpen, onClose, onBookmarkUpdated, bookmark, pools }) => {
  ...
  useEffect(() => {
    if (bookmark) {
      ...
      setUrl(bookmark.url);
      const pool = pools.find(p => p.name === bookmark.category);
      if (pool) setPoolId(pool.id);
    }
  }, [bookmark, pools]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bookmark || !name.trim() || !url.trim()) return;
    
    setLoading(true);
    try {
      await publicBookmarkApi.updateBookmark(bookmark.id, name, url);
      onBookmarkUpdated();
      onClose();
    } catch (err) {
      alert('更新失敗');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="p-6 border-b border-[#e9eaef] flex items-center justify-between">
          <h3 className="text-xl font-bold text-black font-['Epilogue']">編輯書籤</h3>
