...
export const EditPoolModal: React.FC<EditPoolModalProps> = ({ isOpen, onClose, onPoolUpdated, pool }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await publicBookmarkApi.updatePool(pool.id, name);
      onPoolUpdated();
      onClose();
    } catch (err) {
      alert('更新失敗');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="p-6 border-b border-[#e9eaef] flex items-center justify-between">
          <h3 className="text-xl font-bold text-black font-['Epilogue']">編輯類別</h3>
